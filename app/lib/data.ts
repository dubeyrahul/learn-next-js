...
export async function fetchRevenue() {
  try {
    ...
    console.log("Revenue data RowList:");
    console.log(data);
    const revenueData: Revenue[] = data.map((row) => row as Revenue);
...
export async function fetchLatestInvoices() {
  ...
  try {
    const data = await client.sql`
      ...
      LIMIT 5`;
    const revenueData: LatestInvoiceRaw[] = data.map((row) => row as LatestInvoiceRaw);

    const latestInvoices = revenueData.map((invoice) => ({
      ...invoice,
...
export async function fetchFilteredInvoices(
  ...
  try {
    ...
    `;
    const invoices = data.map((row) => row as InvoicesTable);
...
export async function fetchCustomers() {
  try {
    const data = await client.sql`SELECT
      ...
      ORDER BY name ASC
    `;
    const customers = data.map((row) => row as CustomerField);
